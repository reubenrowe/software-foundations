...
Proof. reflexivity. Qed.
  ...
  Compute filter (fun l => (length l) =? 1)
  [ [1; 2]; [3]; [4]; [5;6;7]; []; [8] ].
...
Proof.
  ...
  induction l as [| x l' IH ].
  - reflexivity.
  - simpl. rewrite map_app_distr. rewrite IH. simpl. reflexivity.
Qed.
(** [] *)
...
Definition exists_x {X : Type} (f : X -> bool) (l : list X) : bool :=
  fold (fun x result => result || f x) l false.
Definition forall_x {X : Type} (f : X -> bool) (l : list X) : bool :=
  fold (fun x result => result && f x) l true.
Compute exists_x (fun n => n =? 0) [1;2;3;4;5;6].
Compute forall_x (fun n => n =? 0) [0;0;0].
...
Definition mult (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => 
    (* add_n : X -> X *)
    (* add_n takes in an argument x, and applies f to it n times *)
    let add_n := n X f in
    (* m takes a function and applies it to an argument m times *)
    (* so this applies the add_n function m times to x *)
    m X add_n x.
