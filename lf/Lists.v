...
Fixpoint oddmembers (l:natlist) : natlist :=
  ...
  | nil    => nil
  | h :: t =>
    let t' := oddmembers t in
    if (even h) then t' else h :: t'
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  ...
  | nil     => O
  | n :: s' =>
    let num := count v s' in
    if (eqb n v) then S num else num
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  ...
  | nil    => nil
  | h :: t =>
    if (eqb h v) then t else h :: remove_one v t
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  ...
  | nil    => nil
  | h :: t =>
    let t' := remove_all v t in
    if (eqb h v) then t' else h :: t'
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  ...
  | n :: s1' =>
    if (eqb O (count n s2))
      then false
      else subset s1' (remove_one n s2)
  end.
(*
  (* How to tell Coq that length (remove_all n s1') <= length s1' *) 
  Fixpoint subset' (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | n :: s1' =>
    if ((count n s1') <? (count n s2))
      then subset' (remove_all n s1') (remove_all n s2)
      else false
  end.
*)
...
Lemma eqnat : forall (n : nat), n =? n = true.
Proof.
  induction n as [| n' IH].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. rewrite eqnat. reflexivity.
  - simpl. rewrite eqnat. reflexivity. 
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | [], [] => true
  | n1::l1', n2::l2' => eqb n1 n2 && eqblist l1' l2'
  | _, _ => false
  end.
...
Proof.
  ...
  induction l as [| h tl IH ].
  - reflexivity.
  - simpl. rewrite eqnat. simpl. apply IH.
Qed.
(** [] *)
...
Proof.
  intros.
  induction s1 as [| m s1' IH].
  - simpl. rewrite eqnat. reflexivity.
  - simpl. destruct (eqb m n).
    * rewrite plus_Sn_m. simpl. apply IH.
    * apply IH.
Qed.
