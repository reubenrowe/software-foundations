...
Proof.
  ...
  replace (n + p) with (p + n)
    by (rewrite add_comm; reflexivity).
  rewrite add_assoc.
  reflexivity.
Qed.
(** [] *)
...
(* Lemma succ_bin2nat''_is_bin2nat''_incr :
...
Proof.
  ...
  - destruct b' as [| b'' | b''] eqn:E.
    * simpl. reflexivity.
    * rewrite <- E. simpl. destruct b' as [| b2''| b2''] eqn:E2.
      + simpl. reflexivity.
      + rewrite E.   
Admitted. *)

Lemma succ_bin2nat_is_bin2nat_incr :
forall b, S (bin_to_nat b) = bin_to_nat (incr b).
Proof.
  intros.
  induction b as [| b' IH | b' IH ].
...
Proof.
  ...
  induction n as [| n' IH ].
  - simpl. reflexivity.
  - simpl. rewrite <- succ_bin2nat_is_bin2nat_incr.
    rewrite IH.
    reflexivity.
Qed.
...
Theorem pred_of_succ : forall n, pred (S n) = n.
Proof.
  intros.
  reflexivity.
Qed.
