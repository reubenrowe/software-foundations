...
Proof.
  ...
  - intros m eq.
    destruct m as [| m' ].
    + discriminate.
    + apply f_equal. apply IH. simpl in eq. apply eq.   
Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2.
    ...
    intros eq'. injection eq' as eq1' eq2'.
...
Proof.
  intros.
  destruct b.
  - destruct (f true) eqn:f_true.
    + rewrite f_true. rewrite f_true. reflexivity.
    + destruct (f false) eqn:f_false.
      * rewrite f_true. reflexivity.
      * rewrite f_false. reflexivity.
  - destruct (f false) eqn:f_false.
    + destruct (f true) eqn:f_true.
      * rewrite f_true. reflexivity.
      * rewrite f_false. reflexivity.
    + rewrite f_false. rewrite f_false. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros n.
  induction n as [| n' IH ].
  - intros m p nm_eq mp_eq.
    destruct m.
    + apply eqb_true in mp_eq. rewrite <- mp_eq. reflexivity.
    + apply eqb_true in nm_eq. discriminate.
  - intros m p nm_eq mp_eq.
    destruct m.
      + apply eqb_true in nm_eq. discriminate.
      + destruct p.
        * apply eqb_true in mp_eq. discriminate.
        * simpl in nm_eq.
          simpl in mp_eq.
          simpl.
          apply IH with m.
...
Proof.
  ...
  - intros lf filter_result.
    simpl in filter_result. discriminate.
  - intros lf. simpl.
    destruct (test y) eqn:test_y.
    + intros result. injection result as head tail.
      rewrite <- head. apply test_y.
    + apply IH.
Qed.
(** [] *)
...
Proof.
  ...
  - unfold existsb'. simpl.
    destruct (test x) eqn:test_result.
    + reflexivity.
    + simpl. rewrite IH. unfold existsb'. reflexivity.    
Qed.
